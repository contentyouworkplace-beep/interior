const { createClient } = require('@supabase/supabas      const { data: invoices } = await supabase
        .from('invoices')
        .select(`
          invoice_number,
          title,
...
async function setupInvoices() {
  ...
  try {
    ...
    if (existingInvoices && existingInvoices.length > 0) {
      ...
      invoices?.forEach(inv => {
        const clientName = inv.client ? `${inv.client.first_name} ${inv.client.last_name}` : 'No Client'
        console.log(`  � ${inv.invoice_number}: ${inv.title} (${inv.status}/${inv.payment_status}) - ₹${inv.total_amount} - ${clientName}`)
      })
      return
    }
